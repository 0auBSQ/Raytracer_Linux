...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/02 17:42:17 by rmarracc          #+#    #+#             */
/*   Updated: 2019/04/06 17:52:38 by rmarracc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_scal		fade(t_scal t)
{
	return (t * t * t * (t * (t * 6 - 15) + 10));
}

t_scal		lerp(t_scal t, t_scal a, t_scal b)
{
	return (a + t * (b - a));
}

t_scal		grad(int hash, t_scal x, t_scal y, t_scal z)
{
	int		h;
	t_scal	u;
	t_scal	v;
...
void		noise_params(int *u, t_scal *x, t_scal *y, t_scal *z)
{
	u[0] = (int)floor(*x) & 255;
	u[1] = (int)floor(*y) & 255;
	u[2] = (int)floor(*z) & 255;
...
t_scal		noise(t_scal x, t_scal y, t_scal z)
{
	...
	noise_params(&u[0], &x, &y, &z);
	c[0] = fade(x);
	c[1] = fade(y);
	c[2] = fade(z);
	h[0] = g_perm[u[0] % 256] + u[1];
	h[1] = g_perm[(u[0] + 1) % 256] + u[1];
	h[2] = g_perm[h[0] % 256] + u[2];
	h[3] = g_perm[h[1] % 256] + u[2];
	h[4] = g_perm[(h[0] + 1) % 256] + u[2];
	h[5] = g_perm[(h[1] + 1) % 256] + u[2];
	return (lerp(c[2], lerp(c[1], lerp(c[0], grad(g_perm[h[2] % 256], x, y, z),
